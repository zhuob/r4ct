...
take_snapshot <- function(by = c("time", "event"), ...){
  ...
  by <- match.arg(by)
  dummy <- structure(list(), class = by) # class "event" or "time"
...
#'
#' @param dummy Dispatched S3 selector; not user-supplied
...
#' @param ... absorbs unused arguments from the generic
#'
...
take_snapshot.event <- function(
  ...
  # guard against n_event > number of subjects 
  if (length(snapshot_time) == 0 || is.na(snapshot_time)){
    stop("n_event (", n_event, ") exceeds the number of observed events.")
  }
  
  # delegate to the time-based method (keep dispatched object first)
...
take_snapshot.time <- function(
  ...
  event_flag <- (survival_time <= dropout_time)  & 
                (time_on_calendar <= snapshot_time)
  ...
  dropout_flag <- (event_flag == FALSE) & 
                  (survival_time >= dropout_time) & 
